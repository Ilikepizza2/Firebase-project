...
const Transactions: NextPage = () => {
      ...
      <>
        ...
        <div className="pagination my-10 flex gap-6">
          <div className="flex gap-2">
            <button
              className="w-8 rounded-xl bg-slate-300 hover:bg-slate-500 dark:bg-slate-500 dark:hover:bg-slate-300"
              onClick={() => gotoPage(0)}
  ...
  React.useEffect(() => {
    const firestoreArr = firebase.firestore().collection('Transactions')
  ...
  React.useEffect(() => {
    const realtimeArr = firebase.database().ref('Transactions')
    realtimeArr.on('value', (snapshot) => {
      const items: any[] = []
      ...
      setRealtimeData(
        items.map((transaction) => ({
          ...
          requeststatus: 'Realtime Database',
          walletAction: 'Realtime Database',
          stringDate: 'Realtime Database',
        }))
      )
    })
  }, [])
  React.useEffect(() => {
    setData((prev: any) => [...prev, ...firestoreData])
    setData((prev: any) => [...prev, ...realtimeData])
    setFetched((prev: any) => [...prev, ...firestoreData])
    setFetched((prev: any) => [...prev, ...realtimeData])
    setLoading(false)
  }, [firestoreData, realtimeData])
  ...
  return (
    <div className={'h-100 mr-auto'}>
      <Navbar />
      <h1 className="table-heading">Transactions</h1>
